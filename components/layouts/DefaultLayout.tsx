...
const DefaultLayout: React.FC<Props> = ({ title, children }) => {
    ...
    return (
        <div className="relative flex min-h-screen w-full flex-col items-center justify-between  bg-shop-accents">
            ...
            <footer className="z-10 w-full border-t bg-shop-orange/10 p-4 py-8 pb-0 text-slate-200">
                <div className="container mx-auto grid grid-cols-10 border-shop-orange px-4">
            ...
            <AnimatePresence>
                {toggleLogin && <SignIn error={error} />}
            </AnimatePresence>
            {loading && <Loading />}
            <ToastContainer />
            {/*  <div
